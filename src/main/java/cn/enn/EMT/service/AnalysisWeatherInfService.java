...
public class AnalysisWeatherInfService {
	public  WeatherInfVO analysisWeatherInf(String strPar) {
		...
		try{
			...
			JSONObject index4 = index.getJSONObject(4);//紫外线
		...
		for(int i=0; i<weather_data.size(); i++) {
			...
			//oneDayWeatherInfVO.setDate(calendar.get(Calendar.YEAR) + "." + calendar.get(Calendar.MONTH) + "."+ calendar.get(Calendar.DAY_OF_MONTH));
			...
			if(i == 0) {//当天的天气
				...
				if(beginIndex>-1) {
					...
					oneDayWeatherInfVO.setTemperatureNow(" ");
					oneDayWeatherInfVO.setWeek(OneDayWeatherinfvo.getString("date").substring(0,2));
				}
			} else {
			...
			oneDayWeatherInfVO.setWeather("weather");
